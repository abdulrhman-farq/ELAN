  ...
  return (
    <section className="space-y-6 p-6">
      <header className="flex items-center justify-between">
        ...
        <Link
          href="/profile"
          className="flex h-12 w-12 items-center justify-center rounded-full border border-primary-200 bg-primary-100 font-display text-lg text-primary"
        >
          {initial}
        </Link>
      </header>
      ...
      {next ? (
        <Link href="/bookings" className="card-ink block p-6">
          ...
          <span className="mt-5 inline-block rounded-[2px] bg-primary px-5 py-2.5 text-sm text-surface">{t.home.viewDetails}</span>
        </Link>
      ) : (
        <div className="card-ink p-6">
          <p className="eyebrow text-primary-200">{t.home.nextClass}</p>
          <p className="mt-3 font-display text-xl">{t.home.none}</p>
          <Link href="/schedule" className="mt-4 inline-block rounded-[2px] bg-primary px-5 py-2.5 text-sm text-surface">{t.timetable.title}</Link>
        </div>
      )}

      <div className="flex gap-3">
        <div className="card flex-1 p-4">
          <div className="font-display text-3xl text-primary">{ctx.balance}</div>
          <div className="text-[12px] text-status-full">{t.home.balance}</div>
        </div>
        <div className="card flex-1 p-4">
          <div className="font-display text-3xl text-primary">{attended}</div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link href="/schedule" className="text-[12px] text-primary">{t.home.all}</Link>
        </div>
        {discover.length === 0 ? (
          ...
          <div className="flex gap-3">
            {discover.map((c) => (
              <Link key={c.id} href={`/class/${c.id}`} className="card flex-1 p-4">
                <div className="mb-3 h-16 w-full rounded-[4px] bg-gradient-to-br from-primary-100 to-primary-300" />
