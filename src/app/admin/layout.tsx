...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div dir={dirFor(locale)} className="mx-auto flex min-h-screen max-w-[1200px] flex-col md:flex-row">
      <aside className="flex flex-col gap-8 border-b border-outline bg-brand p-7 text-primary-900 md:w-[230px] md:shrink-0 md:border-b-0 md:border-e">
        <div className="wordmark text-3xl">ÉLAN</div>
        <AdminNav items={nav} />
        <div className="mt-auto flex items-center gap-3 border-t border-outline pt-4">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary font-display text-base text-ink">م</div>
          <div>
            <div className="text-[13px] text-primary-900">{ar ? "مديرة الاستوديو" : "Studio manager"}</div>
            <div className="text-[11px] text-status-full">{ar ? "الرياض" : "Riyadh"}</div>
          </div>
        </div>
        <Link href="/" className="text-sm text-primary-900/60 hover:text-primary-900">{ar ? "التطبيق ›" : "App ›"}</Link>
      </aside>
